...
class Button {
	constructor(a){
		...
		this.el.style.font = `600 18px Lato, sans-serif`
...
class Range {
	...
	constructor(a){
		...
		this.el = document.createElement('div')
		this.el.classList.add(`ss_rangeRail`)
		...
		this.control.addEventListener(`input`, e=>{
			this.handleChange(e)
			// if (this.trigger !== `release`) this.handleTrigger
		})
	...
	update(v){
		...
		if (this.value > this.max) {this.value = this.max} else if (this.value < this.min) {this.value = this.min}
