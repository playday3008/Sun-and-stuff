...
class Core{
	...
	update(){
		...
		this.ff = this.f * (this.divisions - 2) ** 2 % 1
		this.n = Math.floor((this.divisions - 2) ** 2 * this.f)
	...
	drawFrame(){
		...
		let dx = this.unit * (this.n % (this.divisions - 2))
		let dy = this.unit * Math.floor(this.n / (this.divisions - 2))

		if (this.ff > .8){
			dx += ((this.n + 1) % (this.divisions - 2)) ? this.unit * (this.ff - .8) * 5 : -this.unit * (this.divisions - 3) * (this.ff - .8) * 5
			dy += (this.n + 1 === (this.divisions - 2) ** 2) ? -this.unit * (this.divisions - 3) * (this.ff - .8) * 5 : ((this.n + 1) % (this.divisions - 2)) ? 0 : this.unit * (this.ff - .8) * 5
		}
	...
	drawBacks(){
		...
		for (let i = 0; i < this.divisions ** 2; i++){
			...
			if (x > 0 && y > 0 && x < this.unit * (this.divisions - 1) && y < this.unit * (this.divisions - 1)){
		...
		this.ctx.translate(this.leftMargin * 2 + this.cellSize + this.unit, this.topMargin + this.unit)
		
		for (let i = 0; i < (this.divisions - 2) ** 2; i++){

			this.ctx.fillStyle = i > this.n ? this.color.base : this.color.frame
			this.ctx.fillRect(this.unit * (i % (this.divisions - 2)) + this.hair, this.unit * Math.floor(i / (this.divisions - 2)) + this.hair, this.unit - this.hair * 2, this.unit - this.hair * 2)
		}
