...
window.onload = _=>{
	...
	const p = new Range({width: 280, value: 1, class: `input`, min: 1, max: 4, scale: 3, label: `Pooling size`, labelWidth: 120, step: 1})
	...
	const a = new Button({n: `Average`, class: `go`})
	const m = new Button({n: `Max`, class: `go`})
	...
	const p_label = document.createElement(`div`)
    p_label.classList.add(`label`)
    p_label.innerHTML = `Function`
    document.body.appendChild(p_label)
    ...
    document.body.appendChild(rgb_label)
	...
	core = new Core(document.querySelector(`.canvas`), document.querySelector(`.original`), document.querySelector(`.source`))
 ...
 //    preset_label.innerHTML = `Presets`
	...
	rgb.data.onchange = v=>{
		core.channels = 0
		handleButtons(rgb, [r,g,b])
	}
	r.data.onchange = v=>{
		core.channels = 1
		handleButtons(r, [rgb,g,b])
	}
	g.data.onchange = v=>{
		core.channels = 2
		handleButtons(g, [r,rgb,b])
	}
	b.data.onchange = v=>{
		core.channels = 3
		handleButtons(b, [r,g,rgb])
	}
...
class Core{
	...
	constructor(canvas, original, source) {

		this.source = source
		this.canvas = canvas
		this.ratio = 1//window.devicePixelRatio || 1
		this.canvas.width = this.w = 204//this.canvas.clientWidth * this.ratio
		this.canvas.height = this.h = 204//this.canvas.clientHeight * this.ratio
	...
	setListeners(){
		...
		// 	this.imageCell.x = Math.floor(e.offsetX / 4)
		// 	this.imageCell.y = Math.floor(e.offsetY / 4)
	...
	pool(pixels){
		...
		const ws = this.w / this.pooling
		const out = this.ctx.createImageData(ws, this.h / this.pooling)

		const p = pixels.data

		for (let y = 0; y < this.h - 1; y += this.pooling){
			for (let x = 0; x < this.w - 1; x += this.pooling){


				const i = y * this.w * 4 + x * 4
				const is = y / this.pooling * ws * 4 + x / this.pooling * 4
				...
				for (let j = 0; j < 3; j++){
					...
					for (let yy = 0; yy < this.pooling; yy++){
						for (let xx = 0; xx < this.pooling; xx++){

							const ii = i + xx * 4 + yy * this.w * 4
							if (this.f === `max`){

								if (p[ii + j] > values[j]) values[j] = p[ii + j]
					...
					if (this.f === `average`) values[j] /= this.pooling ** 2
		...
		let d = (this.w - this.w / this.pooling) / 2
		this.ctx.putImageData(out, d, d)
		// this.ctx.drawImage(this.canvas, 0, 0, this.w * this.stride, this.h * this.stride)
		// console.log(`Finished in ${(performance.now() - timer)} ms`)
