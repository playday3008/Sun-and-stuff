...
class Core{
	...
	update(){
		...
		this.ff = this.f * (this.divisions - 4) ** 2 % 1
		this.n = Math.floor((this.divisions - 4) ** 2 * this.f)
	...
	drawFrame(){
		...
		let dx = this.unit * (this.n % (this.divisions - 4)) * this.stride
		let dy = this.unit * Math.floor(this.n / (this.divisions - 4)) * this.stride

		if (this.ff > .8){
			dx += ((this.n + 1) % (this.divisions - 4)) ? this.unit * this.stride * (this.ff - .8) * 5 : -this.unit * (this.divisions - 3) * (this.ff - .8) * 5
			dy += (this.n + 1 === (this.divisions - 4) ** 2) ? -this.unit * (this.divisions - 3) * (this.ff - .8) * 5 : ((this.n + 1) % (this.divisions - 4)) ? 0 : this.unit * this.stride * (this.ff - .8) * 5
		}
	...
	drawBacks(){
		...
		this.ctx.translate(this.leftMargin, this.topMargin)
		for (let i = 0; i < this.divisions ** 2; i++){

			const x = this.unit * (i % this.divisions)
			const y = this.unit * Math.floor(i / this.divisions)

			this.ctx.fillStyle = this.color.base
			this.ctx.fillRect(x + this.hair, y + this.hair, this.unit - this.hair * 2, this.unit - this.hair * 2)
			
			if (x > 0 && y > 0 && x < this.unit * (this.divisions - 1) && y < this.unit * (this.divisions - 1) && (x % (this.unit * 2)) && (y % (this.unit * 2))){
		...
		this.ctx.translate(this.leftMargin * 2 + this.cellSize + this.unit * 2, this.topMargin + this.unit * 2)
		
		for (let i = 0; i < (this.divisions - 4) ** 2; i++){
			this.ctx.fillStyle = i > this.n ? this.color.base : this.color.frame
			this.ctx.fillRect(this.unit * (i % (this.divisions - 4)) + this.hair, this.unit * Math.floor(i / (this.divisions - 4)) + this.hair, this.unit - this.hair * 2, this.unit - this.hair * 2)
		}
